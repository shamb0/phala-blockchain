...
use contracts::{AccountIdWrapper, Contract, ContractId, DATA_PLAZA, BALANCE, ASSETS, SYSTEM, WEB3_ANALYTICS, HELLO_WORLD, SECRETB64CODE};
...
struct RuntimeState {
    ...
    contract6: contracts::secretb64code::SecretB64Code,
...
lazy_static! {
    static ref STATE: SgxMutex<RuntimeState> = {
        SgxMutex::new(RuntimeState {
            ...
            contract6: contracts::secretb64code::SecretB64Code::new(),
...
fn query(q: types::SignedQuery) -> Result<Value, Value> {
    ...
    let res = match opaque_query.contract_id {
        ...
        SECRETB64CODE => serde_json::to_value(
            state.contract6.handle_query(
                accid_origin.as_ref(),
                types::deopaque_query(opaque_query)
                    .map_err(|_| error_msg("Malformed request (secretb64code::Request)"))?.request)
        ).unwrap(),
